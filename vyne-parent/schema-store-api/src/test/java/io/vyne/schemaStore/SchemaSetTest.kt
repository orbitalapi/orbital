...
class SchemaSetTest {
   ...
   fun when_addingNewSchema_then_generationIsIncremented() {
      ...
      schemaSet = schemaSet.add(VersionedSchema("foo", "0.1.2", ""))
      schemaSet.generation.should.equal(0)
      schemaSet = schemaSet.add(VersionedSchema("bar", "0.1.2", ""))
   ...
   fun when_addingNewSchemaWithHigherVersion_then_existingVersionIsRemoved() {
      var schemaSet = SchemaSet.EMPTY.add(VersionedSchema("foo", "0.1.0", ""))
      ...
      schema.version.should.equal("0.1.2")
   }

   @Test
   fun when_addingNewSchemaWithLowerVersion_then_existingVersionIsRetained() {
      val originalSchemaSet = SchemaSet.EMPTY.add(VersionedSchema("foo", "0.1.0", ""))
      originalSchemaSet.size().should.equal(1)

      val schemaSet = originalSchemaSet.add(VersionedSchema("foo", "0.0.8", ""))
   ...
   fun when_constructingSchemaSetWithMultipleVersions_then_onlyLatestIsTaken() {
      val schemaSet = SchemaSet.from(
         listOf(
            VersionedSchema("foo", "0.1.0", ""),
            VersionedSchema("foo", "0.1.1", ""),
            VersionedSchema("bar", "0.1.1", "")
         ), 1
      )
