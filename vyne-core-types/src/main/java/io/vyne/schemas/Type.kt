...
data class Type(
   ...
   val enumTypedInstances: List<TypedEnumValue> by lazy {
      this.enumValues.map { enumValue ->
         TypedEnumValue(this, enumValue, this.typeCache)
      }
   }
...
//   }
   ...
   fun enumValue(value: Any): EnumValue {
      return this.enumValues.firstOrNull { it.value == value }
         ?: error("Value $value is not defined on enum ${this.fullyQualifiedName}")
   }
}
