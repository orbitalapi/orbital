...
class JsonModelParser(val schema: Schema, private val mapper: ObjectMapper = DEFAULT_MAPPER) {
   ...
   private fun parseScalarValue(valueMap: Map<String, Any>, type: Type, conversionService: ConversionService, source:DataSource): TypedInstance {
      ...
      return if (type.isEnum) {
         ...
         TypedValue.from(type, value, conversionService, source)
      }
   }
}
