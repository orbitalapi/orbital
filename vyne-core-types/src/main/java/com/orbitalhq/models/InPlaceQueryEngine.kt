...
interface InPlaceQueryEngine : FactBag, QueryContextSchemaProvider {
   ...
   suspend fun evaluate(expression: Expression, facts: FactBag = FactBag.empty(), source:DataSource = Provided): TypedInstance
   suspend fun evaluate(expression: Expression, value:TypedInstance, source:DataSource = Provided): TypedInstance
