---
title: Sample Instrument Service
description: >-
...
Once you've <Link to="../setting-up-vyne-locally">set-up Vyne locally</Link>, we can start building services that can leverage Vyne's automated integration. Vyne is built with Spring Boot, and though it's not a requirement to use spring boot, it's the easiest way to get going.
...
For the app to connect to Vyne we need to specify hazelcast.config location \(refer to <Link to="../setting-up-vyne-locally">set-up Vyne locally</Link>\)

This is to allow your app to join the Hazelcast cluster created by Vyne.

<Tabs tabsNav={["Command Line", "Intellij"]}>
<TabsContent>
