...
class CaskServiceTest  {
    ...
    fun testRequestedTypeNotFound() {
        ...
        val expectedError = Either.right(CaskService.TypeError("Type reference 'WrongType' not found."))
    ...
    fun testEmptySchema() {
        ...
        val expectedError = Either.right(CaskService.TypeError("Empty schema, no types defined."))
        service.resolveType("WrongType").should.equal(expectedError)
    }
    ...
    fun testRequestedTypeValid() {
        val service = CaskService(schemaProvider, ingesterFactory)
        service.resolveType("OrderWindowSummary").isLeft().should.equal(true)
    }
