...
class CsvIngesterBenchmarkTest : BaseCaskIntegrationTest() {
   ...
   fun canStreamDataToPostgresOnStart() {
      ...
      Benchmark.benchmark("ingest to db") { stopwatch ->
         ...
         ingester = Ingester(jdbcTemplate, pipeline, UnicastProcessor.create<IngestionError>().sink(), SimpleMeterRegistry())
   ...
   fun canIngestCsvToCask() {
      ...
      val ingesterFactory = IngesterFactory(jdbcTemplate, caskIngestionErrorProcessor, SimpleMeterRegistry())
