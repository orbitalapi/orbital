...
class DataIngestionTests : BaseCaskIntegrationTest() {
   ...
   fun canIngestWithTimeType() {
      ...
      val pipelineSource = CsvStreamSource(source.byteInputStream(), timeType, schema, MessageIds.uniqueId(), csvFormat = CSVFormat.DEFAULT.withFirstRecordAsHeader(), ingestionErrorProcessor = caskIngestionErrorProcessor)
   ...
   fun canUpsertWithNoPk() {
      ...
      val pipelineSource = CsvStreamSource(source.byteInputStream(), type, schema, MessageIds.uniqueId(), csvFormat = CSVFormat.DEFAULT.withFirstRecordAsHeader(), ingestionErrorProcessor = caskIngestionErrorProcessor)
...
//         .expectError(PSQLException::class.java)
   ...
   fun `Can downcast incoming Instant as Date or Time`() {
      ...
      val pipelineSource = CsvStreamSource(source.byteInputStream(), downCastTestType, schema, MessageIds.uniqueId(), csvFormat = CSVFormat.DEFAULT.withFirstRecordAsHeader(), ingestionErrorProcessor = caskIngestionErrorProcessor)
   ...
   fun `Can Ingest With Default Values`() {
      ...
      val pipelineSource = CsvStreamSource(source.byteInputStream(), modelWithDefaults, schema, MessageIds.uniqueId(), csvFormat = CSVFormat.DEFAULT.withFirstRecordAsHeader(), ingestionErrorProcessor = caskIngestionErrorProcessor)
   ...
   fun `Can Ingest BY Concatenating Values`() {
      ...
      val pipelineSource = CsvStreamSource(source.byteInputStream(), concatModel, schema, MessageIds.uniqueId(), csvFormat = CSVFormat.DEFAULT.withFirstRecordAsHeader(), ingestionErrorProcessor = caskIngestionErrorProcessor)
...
//
   ...
   fun `Instant Formatting`() {
      ...
      val pipelineSource = CsvStreamSource(source.byteInputStream(), instantModel, schema, MessageIds.uniqueId(), csvFormat = CSVFormat.DEFAULT.withFirstRecordAsHeader(), ingestionErrorProcessor = caskIngestionErrorProcessor)
   ...
   fun `Ingestion of scientific numbers`() {
      ...
      val pipelineSource = CsvStreamSource(source.byteInputStream(), decimalModel, schema, MessageIds.uniqueId(), csvFormat = CSVFormat.DEFAULT.withFirstRecordAsHeader(), ingestionErrorProcessor = caskIngestionErrorProcessor)
