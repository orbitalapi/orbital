...
class CsvBinaryReader {
   fun readAllValuesAtColumn(input: Path, columnIndices: Set<Any>): Flux<Map<Int, String>> {
      ...
      columnIndices.forEach {
         when (it) {
            ...
            is String -> sortedIndices.add(header.columnMap?.get(it)!!)
            else -> log().error("Indices can be type of either Int or String.")
         }
      }
      sortedIndices.sorted().toSortedSet()
