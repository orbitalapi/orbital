...
export class AuthService {
  ...
  private async configureOAuthService(): Promise<boolean | null> {
    const authConfig: AuthConfig | null = await this.buildAuthConfig();
    if (authConfig != null) {
  ...
  private async buildAuthConfig(): Promise<AuthConfig | null> {
    ...
    if (frontendConfig === undefined || !frontendConfig.enabled) {
      return null;
    }
  ...
  private loadFrontendConfig(): Observable<FrontendConfig> {
    return this.http.get<FrontendConfig>(`${environment.serverUrl}/api/security/config`);
  }
