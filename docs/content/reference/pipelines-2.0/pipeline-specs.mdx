...
|`targetTypeName`|The fully qualified name of the type which content being pushed to the database should be read as. Pass * to use the type from a querySpec|true||
...
|`querySpec`|If defined, the query return type is used as the target type.|false||
