 ...
 * Tutorial in configuring Vyne with Keycloak - a popular open source OpenID platform
...
writing any code, and without writing any code, and without updating
the publisher applications.
...
It's now possible for large-scale deployments to move capturing of history out to a seperate server - called the Vyne Analytics Server, which renders
 ...
 * Pipeline sink and source from SQS, and SNS
 ...
 * <Link to="/reference/schema-server/schema-server/#publishing-external-schemas">Publishing schemas reference documentation</Link>
 * <Link to='/how-to-guides/register-schemas/publish-a-schema-in-spring-boot/'>Dedicated Spring Boot tutorial</Link>.
 * <Link to="/background/schema-publication-methods/#publishing-schemas-to-vyne">Comparing the different schema publication approaches</Link>
