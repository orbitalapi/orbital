...
Casks can now be configured to output messages to Kafka as they are ingested.  Either delta messages (for casks
with a `@PrimaryKey`) or full models can be output.
