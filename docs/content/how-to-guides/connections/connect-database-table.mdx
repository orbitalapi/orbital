...
Tables are exposed to Vyne using the annotation `@com.orbitalhq.jdbc.Table` on a model.
...
service must be declared, and annotated with the `@com.orbitalhq.jdbc.DatabaseService` annotation.
