...
Paste in the below query, and hit run.
...
Paste the below query into the Query Editor, and hit run.
...
Vyne handles working out how to call the REST API, which data to pass, and what to collect.  Paste the below
query and click Run.
...
When the query results are returned, as this is nested data, click the tree icon to view the results
in tree mode.
...
showing the trace of how the value was derived.  This shows all operations that were called to
fetch this value.
...
Although Vyne's UI is powerful, most developers will want to interact with Vyne through it's API.
That's a topic on it's own, but here is an example f running the same query through Vyne's API, using curl.

```shell
curl 'http://localhost:9090/api/taxiql' \
