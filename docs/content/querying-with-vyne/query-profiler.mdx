...
Note: This must be enabled via the `vyne.history.persist-results` setting.  See query-history for more details.
...
Clicking on any of the rows shows the actual query that was sent and response received.  (requires `vyne.history.persist-remote-call-responses` to be enabled)
