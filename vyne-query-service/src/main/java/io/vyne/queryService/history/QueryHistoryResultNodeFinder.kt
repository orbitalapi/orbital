      ...
      val thisValue = when {
         thisPart.matches(numberInSquareBracketsRegex) -> {
            ...
            if (valueToParse is List<*>) {
               valueToParse[index]
                  ?: throw InvalidPathException("Array index $index is out of bounds on array with size ${valueToParse.size}")
            } else {
               ...
               when (valueToParse.value) {
                  ...
                  is Map<*, *> -> {
                     ...
                     typeNamedInstanceValue[thisPart]
                        ?: throw InvalidPathException("Found a null value at path part $thisPart")
                  }
