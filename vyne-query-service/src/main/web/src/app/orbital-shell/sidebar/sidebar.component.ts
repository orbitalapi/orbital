...
@Component({
  ...
  template: `
    <nav aria-label="Sidebar" class="hidden md:block md:flex-shrink-0 md:overflow-y-auto bg-midnight h-full">
      <div class="relative flex items-center w-28 flex-col space-y-3 p-3">
        <a *ngFor="let element of sidebarElements"
           [routerLink]="element.route"
           [tuiHint]="element.title"
           tuiHintDirection="right"
           class="bg-midnight text-white flex-shrink-0 inline-flex items-center justify-center h-14 w-14 rounded-lg mb-4">
          <div class="flex items-center flex-col">
            <tui-svg [src]="element.icon" class="h-6 w-6"></tui-svg>
...
export class SidebarComponent {
  ...
  readonly sidebarElements: SidebarElement[] = [
    {
      title: 'Catalog',
      icon: '../assets/img/tabler/book.svg',
      route: 'catalog',
      requiredAuthority: VynePrivileges.BrowseCatalog
    },
    {
      title: 'Schemas',
      icon: 'assets/img/tabler/file-code.svg',
      route: 'schemas',
