...
import { vyneQueryLanguageConfiguration, vyneQueryLanguageTokenProvider } from '../query-panel/query-editor/vyne-query-language.monaco';
...
export class CodeEditorComponent implements OnInit {
    ...
    ngOnInit(): void {
      this.monacoLoaderService.isMonacoLoaded$.pipe(
        ...
        const webSocket = this.createWebSocket(`ws://localhost:9500/lsp`)
        listen({
          webSocket,
    ...
    getConfiguration(language: string) {
      switch(language) {
        case "taxi": return { configuration: taxiLanguageConfiguration, tokenProvider: taxiLanguageTokenProvider}
        case "vyneQL": return { configuration: vyneQueryLanguageConfiguration, tokenProvider: vyneQueryLanguageTokenProvider}
      }
...
export type CodeEditorOptions = MonacoEditorConstructionOptions
