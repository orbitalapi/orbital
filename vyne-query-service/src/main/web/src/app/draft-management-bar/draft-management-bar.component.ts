...
export class DraftManagementBarComponent implements OnInit {
  ...
  constructor(public typesService: TypesService, private snackBar: MatSnackBar) {
  }
  ...
  commitChanges() {
    this.typesService.finalizeChangeset().subscribe((response) => {
  ...
  backToMain() {
    this.typesService.selectDefaultChangeset();
  }
