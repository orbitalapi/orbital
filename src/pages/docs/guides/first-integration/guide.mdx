...
wget https://gitlab.com/vyne/demos/-/raw/master/films/docker/schema-server/schema-server.conf -P orbital/schema-server
wget https://gitlab.com/vyne/demos/-/raw/master/films/docker/schema-server/projects/taxi.conf -P orbital/schema-server/projects
wget https://gitlab.com/vyne/demos/-/raw/master/films/services.conf -P orbital/config
docker compose up -d
```
...
| orbitalhq/schema-server   | Orbital                     | A schema server, which stores all the schemas that Orbital knows about                                                                                                             |
...
| Table             | `film`                                                                              |
| Default namespace | `com.orbitalhq.demos.films`                                                             |
...
<Callout title='Related links' type='note'>
   ...
   * [Understanding the different ways to publish schemas to Orbital](/docs/connecting-data-sources/schema-publication-methods)
   * [Describing REST APIs in Taxi](https://docs.taxilang.org/language-reference/describing-services/)
   * [Embedding Taxi definitions inside Swagger, to keep Orbital automatically up-to-date](https://docs.taxilang.org/generating-taxi-from-source/#openapi-x-taxi-type-extension)
...
<Callout title='Related links' type='note'>
   ...
   * [Writing queries with Orbital](../../querying-with-vyne/writing-queries/)
...
<Callout type='note' title="Better json with jq">
...
Alternatively, for queries with large datasets, Orbital can stream results.  This enables Orbital to work on
arbitrarily large datasets, as it's not holding content in memory.

To ask Orbital to stream the data over HTTP, Orbital can serve [Server-Sent events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events).
