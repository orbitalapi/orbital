 ...
 * Generating code for an API client
 * Tracking down the inputs needed
 * Doing any additional calls to resolve ids etc.
 * Combinging together multiple calls to fetch the data I need
...
And, it's really brittle.  Each painstaking integration will inevitably break as things change, causing each team to go and repair their glue.
...
Fundamentally, data across the wire is just `Map<String,Any>`.  API contracts are great at telling us what field names to expect, but they don't really do a good job of describing what information I'll find there.


### Field names are a bad proxy for Semantics
Field names give developers a hint as to what information they'll find in each field.  However, they're pretty hit & miss, and they don't form any sort of semantic contract.  We have fields called `customerId` - 
...
Many teams waste countless cycles trying to implement this, and it almost never yeilds results.  Teams embrace microservices so they can evolve independently.  Requiring a single global
...
<ImageWithCaption src={untypedDiagram.src} caption={`3 systems, but without any hints as to how it all relates`} wide  voyagerLink={`DCb8cYYKmj`}  />
...
<ImageWithCaption src={withSemanticTypes.src} caption={`By embedding semantic types, we can infer how data relates`} wide voyagerLink={`ChLGny26S6`} />
...
Taxi provides a query language for asking for data, and [Orbital](https://orbitalhq.com) is an integraiton platform that understands Taxi queries.
...
<ImageWithCaption src={mermaidDiagram.src} caption={`Semantic metadata tells us enough to automate the integration`} />
...
For example, we could rename the `ShoppingCart.custId` field to `customerId`:
