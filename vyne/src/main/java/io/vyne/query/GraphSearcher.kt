...
//      val factTypes = knownFacts.map { it.type }.toSet()
      ...
      while (nextPath != null) {
         ...
         val resultSatisfiesConstraints = pathEvaluatedSuccessfully && invocationConstraints.typedInstanceValidPredicate.isValid(resultValue)
...
class EvaluatedPathSet {
   ...
   fun visitedCountAsCost(fromState: Element, action: Relationship, toState: Element): Double {
      val hashableTransition = HashableTransition(fromState, action, toState)
      val travsersedCount = transitionCount.getOrDefault(hashableTransition, 0)
   ...
   val traversedEdges = paths.values.flatMap { weightedNode ->
      weightedNode.path()
