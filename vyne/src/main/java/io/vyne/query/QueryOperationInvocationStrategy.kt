...
class QueryOperationInvocationStrategy(
         ...
         when (constraint) {
            is PropertyToParameterConstraint -> if (isCovariant) {
               queryOperation.supportedFilterOperations.contains(constraint.operator)
                  && returnTypeContainsFieldAssignableToConstraint(schema, constraint, queryOperation.returnType)
            } else {
               queryOperation.supportedFilterOperations.contains(constraint.operator)
            }
            else -> {
   ...
   fun returnTypeContainsFieldAssignableToConstraint(
      schema: Schema,
      propertyToParameterConstraint: PropertyToParameterConstraint,
      operationReturnType: Type
   ): Boolean {
      val propertyConstraintTypeName = propertyToParameterConstraint.propertyIdentifier.taxi
      val propertyConstraintVyneType = schema.type(propertyConstraintTypeName).taxiType
      ...
      val assignableFields = (operationReturnParameterisedType.taxiType as ObjectType).fieldReferencesAssignableTo(
         propertyConstraintVyneType
      )
      return assignableFields.isNotEmpty()
   }
