...
class VyneGraphBuilder(private val schema: Schema) {
   ...
   private val baseSchemaGraph: Pair<HipsterGraphBuilder<Element, Relationship>, Multimap<TypeElement, MemberElement>> by lazy {
      timed("VyneGraphBuilder.buildBaseSchemaGraph") {
         ...
         builder to typesAndWhereTheyreUsed
      }

   }

   fun build(facts: Set<TypedInstance> = emptySet()): HipsterDirectedGraph<Element, Relationship> {
      val (builder, typesAndWhereTheyreUsed) = baseSchemaGraph
      val thisBuilder = builder.copy()
      appendInstances(thisBuilder, facts, schema, typesAndWhereTheyreUsed)
