...
class ServiceBusPublisher(
    ...
    private val batchWriteCacheProvider = BatchWriteCacheProvider<TypedInstance, OperationResultReference>()
     ...
     fun publishToTopic(
        ...
        schema: Schema): Flow<TypedInstance> {
        ...
        return serviceBusConnectionFactory
            ...
            .doOnEach { _ ->
                meterRegistry.counter("orbital.connections.servicebus.${connectionName}.topic.${serviceBusTopicOperation.topic}.messagesPublished").increment()
     ...
     fun publishToQueue(
        ...
        return serviceBusConnectionFactory
            ...
            .subscribeOn(Schedulers.boundedElastic())
            .doOnEach { _ ->
                meterRegistry.counter("orbital.connections.servicebus.${connectionName}.queue.${serviceBusQueueOperation.queue}.messagesPublished").increment()
    ...
    suspend fun batchPublishToTopic(
        ...
        return this.doBatch(
            ...
            schema,
            asyncSender,
            ".topic.${serviceBusTopicOperation.topic}"
        )
    }
    ...
    suspend fun batchPublishToQueue(
        ...
        return this.doBatch(
            ...
            schema,
            asyncSender,
            ".queue.${serviceBusTopicOperation.queue}"
        )
    }

    private suspend fun doBatch(service: Service,
                        operation: RemoteOperation,
                        queryId: String,
                        connectionName: String,
        ...
        val batchWriteCache =  batchWriteCacheProvider.forQueryId(
            ...
            asyncSender
                ...
                .subscribeOn(Schedulers.boundedElastic())
                .doOnEach { _ ->
                    meterRegistry.counter("orbital.connections.servicebus.${connectionName}.$meterRegistryString.messagesPublished").increment(serviceBusMessages.size.toDouble())
                }
                .doOnSubscribe {
