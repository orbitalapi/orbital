...
class DynamoDbRequestBuilder {
   ...
   private fun attributeValue(value: Any?): AttributeValue {
      ...
      return when (value) {
         ...
         is List<*> -> {
            val members = (value as List<TypedInstance>).map { it.toRawObject() }
            when {
               members.isEmpty() -> AttributeValue.builder().ss(emptyList()).build()
               members[0] is String -> AttributeValue.builder().ss(members.map { it.toString() }).build()
               members[0] is Number || value[0] is BigDecimal -> {
                  AttributeValue.builder().ns(members.map { it.toString() }).build()
               }
               members[0] is Map<*,*> -> {
                  val listOfMaps = members.map {nestedObject ->
                     nestedObject as Map<String,Any>
                     attributeValue(nestedObject)
                  }
                  AttributeValue.builder().l(listOfMaps).build()
               }
               else -> error("Cannot construct a DynamoDb ListType from list where first member (used for inference) is type of ${value[0]!!::class.simpleName} - not supported")
            }
         }
         is Map<*,*> -> {
            val memberMap = value as Map<String,Any?>
            AttributeValue.builder().m(memberMap.entries.map { (key,value) ->
               val wrappedValue = if (value is TypedInstance) {
                  attributeValue(value.toRawObject())
               } else {
