...
class TaxiSchemaStoreService(private val validatingStore: LocalValidatingSchemaStoreClient = LocalValidatingSchemaStoreClient()) : SchemaStoreService, SchemaProvider {
   ...
   override fun submitSources(sources: List<VersionedSource>): Mono<SourceSubmissionResponse> {
      return Mono.just(validatingStore.submitSchemas(sources)
   ...
   override fun submitSchema(@RequestBody schema: String, @PathVariable("schemaId") schemaId: String, @PathVariable("version") version: String): Mono<SourceSubmissionResponse> {
   ...
   override fun removeSchema(@PathVariable("schemaId") schemaId: String, @PathVariable("version") version: String): Mono<String> {
      TODO("Removing schema ${schemaId} not implemented")
   }

   @RequestMapping(method = arrayOf(RequestMethod.GET))
   override fun listSchemas(): Mono<SchemaSet> {
