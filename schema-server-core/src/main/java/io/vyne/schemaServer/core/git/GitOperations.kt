...
//      }
   ...
   fun raisePr(branchName: String, description: String, author: String): String {
      ...
      val response = repo.createPullRequest(
         "Update $branchName",
         withPrefix(branchName),
         config.branch,
         "This PR was generated by Vyne as per a draft proposed by $author. Description of changes:\n\n$description"
      )
   ...
   fun createBranch(branchName: String): Boolean {
      return try {
         git.checkout().setCreateBranch(true).setName(withPrefix(branchName)).call()
         true
      } catch (e: Exception) {
   ...
   fun getBranches(): List<String> {
      ...
      val prefix = "refs/heads/" + getCurrentPrefix()
   ...
   private fun getCurrentPrefix(): String {
      return config.updateFlowConfig?.branchPrefix ?: ""
   }
}
