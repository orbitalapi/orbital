...
export class TypesService {
    ...
    getRawSchema = (): Observable<string> => {
        return this.http
            .get<string>(`http://localhost:9022/schemas/raw`)
    };

    getLinksForNode = (node: SchemaGraphNode): Observable<SchemaGraph> => {
        return this.http
            .get<SchemaGraph>(`http://localhost:9022/nodes/${node.type}/${node.nodeId}/links`)
    };
    getLinks = (typeName: string): Observable<SchemaGraph> => {
        return this.http
            .get<SchemaGraph>(`http://localhost:9022/types/${typeName}/links`)
    };
    ...
    getTypes = (): Observable<Schema> => {
        ...
        return this.http
        // .get("api/types")
            .get<Schema>("http://localhost:9022/types")
            .map(schema => {
